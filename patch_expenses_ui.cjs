...
const content = `import React, { useState, useRef, useMemo } from 'react';
...
export const MaintenanceExpenses = ({ cars = [], records = [], setRecords = (val: any[]) => {}, staff }: { key?: string, cars?: any[], records?: any[], setRecords?: (val: any[]) => void, staff?: any }) => {
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (expenseType === 'car_maintenance') {
                ...
                total = Number(baseSalary || 0) + Number(allowances || 0);
                data = {
                    ...data,
            ...
            if (expenseType === 'car_maintenance') {
                ...
                setExpenseTitle(''); setBaseSalary(''); setAllowances(''); setExpenseNote(''); setBeneficiary('');
            } else {
    ...
    }, [records, viewFilter, viewCarId, currentMonth, currentYear]);
    ...
    if (expenseType === 'car_maintenance') {
        ...
        currentTotal = Number(baseSalary || 0) + Number(allowances || 0);
    } else {
        ...
        <div className="dashboard-container" style={{ direction: 'rtl' }}>
            <style>
                {\`
                    ...
                    .metric-card h3 { font-size: 14px; color: var(--text-light); margin: 0 0 10px 0; font-weight: bold; }
                    .metric-card .value { font-size: 24px; font-weight: 800; color: var(--primary); }
                    ...
                    .form-panel h2 { font-size: 18px; margin: 0 0 20px 0; font-weight: 800; color: var(--primary); padding-bottom: 8px; border-bottom: 1px solid var(--border); }
                    ...
                    @media print {
                        ...
                        .app-header button, .form-panel, .btn-action, .print-hidden {
            ...
            <header className="app-header">
                <div>
                    <h1>نظام إدارة صرفيات الشركة والرواتب الذكي</h1>
                    <p style={{ color: 'var(--text-light)', fontSize: '14px', marginTop: '4px', marginBottom: 0 }}>لوحة تحكم ومراقبة التقارير المالية المتوافقة مع الطباعة الموحدة</p>
                </div>
                <button className="btn btn-print print:hidden" onClick={handlePrint}>
                    <Printer size={18} />
                    طباعة التقرير المالي الحالي
                </button>
            </header>
            ...
            <main className="main-content">
                ...
                <div className="form-panel print-hidden">
                    <h2>تسجيل حركات صرف ومستندات جديدة</h2>
